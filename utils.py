...
REPLACE_LINK_SPACE = re.compile("(((http|https)?):((//)|(\\\\))+([\w\d:#@%/;$()~_?\+-=\\\.&](#!)?)*)")
...
def preprocess_text(reviews):
    """Remove punctuation, http links, HTML breaks and lower the text"""
    reviews = [REPLACE_LINK_SPACE.sub(" ", line) for line in reviews]
